...
\begin{answer}
    Since $\by$ is one-hot, only its $o$-th entry contributes: $-\sum_{w\in\text{Vocab}}\by_w\log(\hat{\by}_w)=-\log(\hat{\by}_o)$.
\end{answer}
...
\begin{answer}
    \begin{enumerate}
        ...
        &= \frac{\partial}{\partial \bv_c}\left[-\bu_o^\top\bv_c
        + \log\left(\sum_{w\in\text{Vocab}}\exp(\bu_w^\top\bv_c)\right)\right] \\
        &= -\bu_o + \frac{\sum_{w\in\text{Vocab}}\exp(\bu_w^\top\bv_c)\bu_w}
        {\sum_{w\in\text{Vocab}}\exp(\bu_w^\top\bv_c)} \\
        &= -\bu_o + \sum_{w\in\text{Vocab}}\hat{\by}_w\bu_w \\
        &= \bU(\hat{\by}-\by).
        \end{align*}

        \item
        \begin{align*}
        \frac{\partial \bJ_{\text{naive-softmax}}(\bv_c,o,\bU)}{\partial \bv_c}=\mathbf{0}
        \quad \Longleftrightarrow \quad
        \bU(\hat{\by}-\by)=\mathbf{0}.
        \end{align*}

        \item
        Subtracting $-\bu_o$ moves $\bv_c$ toward the true outside vector $\bu_o$. Subtracting $\sum_w\hat{\by}_w\bu_w$ moves $\bv_c$ away from outside vectors, weighted by their predicted probabilities.
    \end{enumerate}
\end{answer}
...
\begin{answer}
    If $\mathbf{u}_x = \alpha \mathbf{u}_y$ for some $\alpha > 0$, then after L2 normalization we have $\mathbf{u}_x / \|\mathbf{u}_x\|_2 = \mathbf{u}_y / \|\mathbf{u}_y\|_2$, so the information in their magnitudes is lost. This matters when the downstream classification depends on vector length, but it does not matter when the task only depends on direction.
\end{answer}
...
\begin{answer}
    \[
    \frac{\partial \bJ_{\text{naive-softmax}}(\bv_c,o,\bU)}{\partial \bU}
    =
    \begin{bmatrix}
        \dfrac{\partial \bJ(\bv_c,o,\bU)}{\partial \bu_1} &
        \dfrac{\partial \bJ(\bv_c,o,\bU)}{\partial \bu_2} &
        \cdots &
        \dfrac{\partial \bJ(\bv_c,o,\bU)}{\partial \bu_{|\text{Vocab}|}}
    \end{bmatrix}.
    \]
\end{answer}
...
\begin{answer}
    ...
    &= -\bu_o^\top\bv_c
    + \log\left(\sum_{x\in\text{Vocab}}\exp(\bu_x^\top\bv_c)\right), \\
    \frac{\partial \bJ_{\text{naive-softmax}}(\bv_c,o,\bU)}{\partial \bu_w}
    &= -\by_w\bv_c + \hat{\by}_w\bv_c \\
    &= (\hat{\by}_w-\by_w)\bv_c \\
    &=
    \begin{cases}
        (\hat{\by}_o-1)\bv_c, & w=o, \\
        \hat{\by}_w\bv_c, & w\neq o.
    \end{cases}
    \end{align*}
\end{answer}

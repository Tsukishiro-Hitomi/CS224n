...
\begin{parts}
    ...
    \part[4] Adam Optimizer\newline
        ...
        \begin{subparts}
            ...
            \begin{answer}
                By keeping a rolling average, $\mathbf{m}$ acts like physical inertia; when gradients oscillate wildly across minibatches, their opposing components cancel each other out, which dampens the variance in those unstable directions. Conversely, in directions where gradients consistently point toward the minimum, momentum accumulates to accelerate progress. This reduced variance smoothes out the optimization path, preventing the model from getting stuck in erratic oscillations and ultimately leading to faster, more stable convergence.
            \end{answer}
           ...
           \begin{answer}
                Model parameters that have infrequent or small gradients will receive larger updates because dividing by a smaller rolling magnitude ($\sqrt{\mathbf{v}}$) scales up their effective learning rate. This helps with learning by ensuring that features or parameters which rarely receive updates aren't neglected, allowing the model to escape flat regions and progress more uniformly across all dimensions.
           \end{answer}
          ...
          \subpart[2] Why should dropout be applied during training? Why should dropout \textbf{NOT} be applied during evaluation? \textbf{Briefly explain in 2--4 sentences}. \textbf{Hint:} it may help to look at the dropout paper linked. \newline
                \begin{answer}
                    Dropout is applied during training to act as a regularizer by randomly deactivating neurons, which prevents them from co-adapting and forces the network to learn robust, redundant features. However, it should not be applied during evaluation because we want the model to output deterministic, fully reproducible predictions using the combined knowledge of all learned neurons.
                \end{answer}
